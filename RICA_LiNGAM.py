...
class RICA():
    ...
    def fit(self,X,n_components,alpha=1,Lambda=1e+3):
        ...
        self.cost_history = []
        self.W_init = np.random.randn(n_components, n_dim)#self._get_W_init()
...
class RICA_LiNGAM():
    ...
    def fit(self, X,n_components=2, print_result=True,n_iter=1000,
            random_state=0, Lambda=1e+6, alpha=1,
    ...
    def get_prob(self):
        ...
        W_base = self.get_W_base()
        for i in range(self.W_list.shape[1]):
            for j in range(self.W_list.shape[2]):
                mu,var = self.get_single_distribution(self.W_list[:,i,j], W_base[i,j])
                Prob_list[i,j] = self.P_0(mu,var)
                W_new[i,j] = mu
        return Prob_list, W_new

    def get_single_distribution(self,w_list,w_base):
        bic_list = []
        for i in range(1,6):
            gm = GaussianMixture(n_components=i, random_state=self.random_state)
            gm.fit(w_list.reshape(-1, 1))
            bic_list.append(gm.bic(w_list.reshape(-1, 1)))
        i = np.argmin(bic_list)+1
        gm = GaussianMixture(n_components=i, random_state=self.random_state)
        gm.fit(w_list.reshape(-1, 1))
        label = gm.predict(w_list.reshape(-1, 1))
        label_base = gm.predict(w_base.reshape(-1, 1))[0]

        return gm.means_.ravel()[label_base], np.sqrt(gm.covariances_.ravel()[label_base])
    ...
    def matrix_plot(self):
        ...
        for i in range(f1):
            for j in range(f2):
                ...
                sns.distplot(self.W_list[:,int(i%f1),j%f2],bins=int(len(self.W_list)/5))
                p += 1
